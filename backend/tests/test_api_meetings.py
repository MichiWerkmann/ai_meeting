...
def test_submit_meeting_audio_response_has_no_room_metadata(monkeypatch):
    ...
    assert "minutes" not in body  # response is already the minutes object
...
def test_forward_meeting_minutes_skips_action_item_notifications_when_disabled(monkeypatch):
    captured = {"notifications_called": False}
    ...
    def _fake_send_notifications(**_kwargs):
        captured["notifications_called"] = True
        return []

    monkeypatch.setattr("backend.app.main.webhook_service.send_minutes", _fake_send_minutes)
    monkeypatch.setattr(
        "backend.app.main.action_item_notification_service.send_notifications",
        _fake_send_notifications,
    )
    ...
    response = client.post(
        ...
        json={
            ...
            "minutes": {
                ...
                "decisions": [],
                "action_items": [
                    {"owner": "Dennis", "description": "Testaufgabe", "due_date": None}
                ],
                "risks": [],
                "sections": [],
                "model": "test",
    ...
    body = response.json()
    assert body["webhook"]["delivered"] is True
    assert body["action_item_notifications"] == []
    assert captured["notifications_called"] is False


def test_forward_meeting_minutes_passes_action_item_notification_overrides(monkeypatch):
    captured = {}
    ...
    def _fake_send_notifications(**kwargs):
        captured["overrides"] = kwargs.get("overrides")
        return [
            {
                "owner": "Dennis",
                "action_item_description": "Testaufgabe",
    ...
    assert response.status_code == 200
    assert isinstance(captured["overrides"], list)
    assert len(captured["overrides"]) == 1
    assert captured["overrides"][0].recipient_email == "owner@example.com"
    assert captured["overrides"][0].email_body == "Custom body"


def test_forward_meeting_minutes_records_task_board(monkeypatch):
    captured = {}
    ...
    def _fake_record_meeting_tasks(**kwargs):
    ...
    monkeypatch.setattr("backend.app.main.task_board_service.record_meeting_tasks", _fake_record_meeting_tasks)
    ...
    assert response.status_code == 200
    assert captured["owner_id"] == "client-a"
    assert captured["meeting_name"] == "Weekly Sync"
    assert captured["meeting_key"] == "job-123"
    assert captured["notify_action_items"] is True


def test_get_task_board_returns_board_for_client(monkeypatch):
    monkeypatch.setattr(
        "backend.app.main.task_board_service.get_board",
        lambda _owner_id: {
            "entries": [
                {
                    "id": "client-a:1",
                    ...
                    "email_detail": "E-Mail erfolgreich versendet.",
                    "updated_at": 1.0,
                }
            ],
            "analytics": {
    ...
    response = client.get("/api/tasks/board", headers={"X-Client-Id": "client-a"})

    assert response.status_code == 200
    body = response.json()
    assert body["analytics"]["total_tasks"] == 1
    assert body["entries"][0]["meeting_name"] == "Weekly Sync"
